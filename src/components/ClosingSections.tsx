...
export function ProjectsShelf() {
  return (
    <section
      ...
      className="relative overflow-hidden border-b border-(--color-hairline) px-6 py-16 md:px-12 md:py-28"
    >
      <span
        aria-hidden
      ...
      <Reveal delay={0.1}>
        <p className="mt-6 max-w-md leading-relaxed text-(--color-faint)">
        ...
        {VAULT_SLOTS.map((slot, i) => (
          <li key={slot}>
            <Reveal delay={Math.min(i * 0.08, 0.24)} className="h-full">
              <div className="flex h-full min-h-40 flex-col justify-between border border-dashed border-(--color-hairline) p-5 transition-colors duration-300 hover:border-(--color-brass)/60">
...
export function ContactFooter() {
  return (
    <section
      ...
      className="relative overflow-hidden px-6 pt-16 pb-24 md:px-12 md:pt-28"
    >
      <Reveal>
