...
export function AboutSection() {
  return (
    <section
      ...
      className="relative overflow-hidden border-b border-(--color-hairline) px-6 py-16 md:border-r md:border-b-0 md:px-12 md:py-24"
    >
      <span
        aria-hidden
      ...
      <Reveal delay={0.08}>
        <p className="font-display mt-6 text-2xl leading-snug text-(--color-lume)/90 md:text-[1.7rem]">
          Fast, quiet interfaces and the systems behind them.
        </p>
      </Reveal>
      <Reveal delay={0.14}>
        <p className="mt-5 max-w-md leading-relaxed text-(--color-faint)">
      ...
      <Reveal delay={0.2}>
        <p className="mt-8 border-l-2 border-(--color-brass) pl-4 font-mono text-xs leading-relaxed text-(--color-faint)">
    ...
    >
      ...
      >
        {PERIODIC_TILES.map((tile, i) => (
          <li key={tile.symbol}>
            <Reveal delay={Math.min(i * 0.06, 0.36)} className="h-full">
              <div className="group h-full border border-black/50 bg-(--color-tile) p-4 transition-all duration-300 ease-out hover:-translate-y-1 hover:border-(--color-brass) hover:shadow-[0_12px_32px_-12px_color-mix(in_srgb,var(--color-brass)_45%,transparent)]">
